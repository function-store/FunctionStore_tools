...
FNS_ConfigRegistry is one of the toolkit's **core** packages -- always installed, never optional. Core is the six surface registries (Config, Toolbar, Navbar, MainMenu, OpMenu, PaneType) plus [FNS_Console](/docs/fns-console/) and [FNS_Updater](/docs/fns-updater/). It ships as its own package, promoted to `/sys` (global shortcut `op.FNS_CONFIGREGISTRY`). Every tool that wants its settings to persist carries a small **host** copy of it, visible as the tool's own `Registry` page (`Cf`-prefixed pars: `Cfautoload`, `Cfregister`, `Cfregstatus`, etc.) -- removing a tool's host just means that tool's settings stop syncing, nothing else breaks.
