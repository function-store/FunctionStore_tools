...
- The root's `webBrowser` panel (where the console and the picker show
  in TD) renders **only while someone can see it**: the component's own
  *Render Only While Window Open* / *Render Only While Viewer Active*
  watchers follow the floating window and the node's Viewer Active flag,
  and the console and installer switch it on as they open it. Closed, its
  Web Render cooks nothing.
...
| **Local Browser** | optional: the tool's own web browser panel rendering the same page. While the tab is exposed the console serves the page and **switches that browser's `Active` off**; a renderer nobody looks at would keep a CEF process and a texture alive. It switches `Active` back on when exposure ends (Expose off, a failed registration, the host removed). A tool that wants finer control implements `OnConsoleExposure(exposed)` on its extension; the host then calls that and leaves `Active` alone |
...
| `/tools`, `/manifest.js`, `/selection`, `/status`, `/install` | any | forwarded to `FNS_Installer` |
