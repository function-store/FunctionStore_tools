...
summary: 'Switch between any number of TOP scenes with a crossfade, a fade through black, a cut or a hands-on A/B fader, cooking only the scenes you can see. The Pro build adds per-scene timing, a cue table, CHOP output blending and fleet follow.'
features:
  - name: Contents
    anchor: contents
  - name: Quick start
...
The Pro build adds per-scene modes, fade times and easing, a cue table with play, timeline, timecode and CHOP drivers, CHOP output blending, and fleet follow with scene offset and sync timing. It unlocks at the Pro tier or higher and installs in place of the Base build under the same name; the picker lands whichever build your account holds, and an installed Pro build is kept as it is if your tier changes. Base and Pro instances take part in the same fleet.
