...
re-registers rather than restoring.
...
`background` is a **grow** zone: rather than taking its height out of the
...
recomputed as base plus growth, never incremented — the bar's height is saved in
...
final zone settles across a frame rather than asserting in the same pass.
