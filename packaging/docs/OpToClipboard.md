...
Since there's no telling from TD API where the pasting is happening, the pasted text will have an `@` identifier after it, that will automatically resolve to the full relative/shortcut path to the copied OP reference. The base mechanism is the same as [ExprHotStrings](/docs/exprhotstrings/#exprhotstrings) in the end. 
