...
See the extensive description of [QuickExt](/docs/fns-custompartools/).
...
A fast way to customize parameter promotion: name it, set min/max values, and tweak clamping. Related to [CustomParTools](/docs/fns-custompartools/#custompar-tools)
   - `Label` and `ParName` for **all** params, and for slider-type params: `min`, `max` values, `clamping` tickboxes, and `default` value  
   - **Invoked** by holding `Ctrl` while drag-and-dropping a parameter onto the `diamond` button or a parent in the path bar  
   - **Quickly jump** between text fields by pressing `Tab` and `Shift+Tab`  

When adding an extension using [CustomParTools](/docs/fns-custompartools/#quickext)![](/docs/assets/icons/CustomParTools.png) **Shift+Alt+LeftClick**, the created default Extension will have the following available:
- Simplified default extension code
- Includes `ExtUtils` which contains [CustomParHelper](/docs/fns-custompartools/#customparhelper) and [NoNode](/docs/fns-custompartools/#nonode), which are also initialized in the default extension code
...
2. Initialize in your extension's __init__ method as follows:
    ...
    - `enable_stubs`: **No longer does anything from here** (default: False). Stub
      ...
      keep working. Generate stubs through [VSCodeTools](/docs/fns-vscodetools/#stubs) or
...
Everything above is a separate sub-module, and the **Modules** page on the
CustomParTools component switches each one on or off independently. All nine
default to on, and the settings persist through FNS_ConfigRegistry like the rest
...
- **Navbar Drag/Drop** and **Navbar Path Cell Click** each own their own navbar
  ...
  from every pane bar.
- The other five in the top group share one surface, the CustomParTools button.
  It leaves the navbar and the toolbar only when **all five** are off, since any
  one of them still gives the button something to do.
- The settings registration is never withdrawn. It carries no surface, and it is
  what stores these toggles, so dropping it would discard the setting that asked
  for the drop.
