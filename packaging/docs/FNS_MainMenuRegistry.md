...
It ships as its own core package, promoted to `/sys` (global shortcut `op.FNS_MAINMENUREGISTRY`) -- one of the toolkit's six surface registries, which together with [FNS_Console](/docs/fns-console/) and [FNS_Updater](/docs/fns-updater/) make up the always-installed core. A tool that wants a main-menu entry -- for example [BorderlessTD](/docs/borderlesstd/)'s project-name display, or [FNS_MainMenu](/docs/fns-mainmenu/)'s own Configurator gear -- ships a small host copy of this registry alongside its widget. You don't interact with the registry directly: open the [Main Menu Configurator](/docs/fns-mainmenu/#main-menu-configurator) (the gear icon in the bar) to reorder, hide or group entries.
