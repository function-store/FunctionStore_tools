...
It ships as its own core package, promoted to `/sys` (global shortcut `op.FNS_MAINMENUREGISTRY`), one of the toolkit's six surface registries, which together with [Console](/docs/fns-console/), [Hub](/docs/fns-hub/) and [Updater](/docs/fns-updater/) make up the always-installed core. A tool that wants a main-menu entry, for example [BorderlessTD](/docs/fns-borderlesstd/)'s project-name display or the hub's own **FNS** button, ships a small host copy of this registry alongside its widget. You don't interact with the registry directly: open the **MainMenu** tab of [Hub](/docs/fns-hub/#the-tabs) (the FNS button in the bar) to reorder, hide or group entries, TD's own items included.
