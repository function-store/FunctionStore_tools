...
# The root webBrowser renders only while its floating viewer is open. A Web
# Render TOP cooks a CEF process whether or not anyone looks at it, and this
# panel ships to every user inside the bootstrap -- so it must cost nothing
# until the console or the picker opens it. The rule is the panel value
# winopen ("1 if panel is open as a floating window"), watched by a Panel
# Execute DAT beside the browser; the console and the installer also switch
# Active on as they open the viewer, so the page starts loading at once.
BROWSER_WATCH = 'watch_webbrowser'
BROWSER_WATCH_TEXT = (
    '"""The root webBrowser panel -- where the console and the picker show --\n'
    'renders only while its floating viewer is open: a Web Render cooks a CEF\n'
    'process for nobody otherwise. winopen -> Active. Generated by\n'
    'packaging/build_installer.py (EnsureDevRails); the bootstrap keeps it."""\n\n\n'
    'def onValueChange(panelValue, prev):\n'
    '\tpanelValue.owner.par.Active = bool(panelValue)\n'
    '\treturn\n')


def _ensureBrowserWatch(root, web, x, y):
    """The winopen watcher beside `web`, created or brought up to date; the
    browser itself is left dormant unless its viewer is open right now."""
    w = root.op(BROWSER_WATCH)
    if w is None:
        w = root.create(panelexecuteDAT, BROWSER_WATCH)
        w.nodeX, w.nodeY = x, y
    w.par.panels = web.name          # sibling: OP pars resolve from the parent network
    w.par.panelvalue = 'winopen'
    w.par.valuechange = True
    for n in ('offtoon', 'ontooff', 'whileon', 'whileoff'):
        setattr(w.par, n, False)
    w.par.active = True
    w.text = BROWSER_WATCH_TEXT
    p = getattr(web.par, 'Active', None)
    if p is not None:
        try:
            p.val = bool(web.panel.winopen)
        except Exception:
            p.val = False
    return w
...
def _loadWebBrowser(root, x, y):
    ...
    TOP + interaction), vendored at packaging/webBrowser.tox, with its
    winopen watcher. Returns (comp, error)."""
    ...
    # the watcher hugs the browser like a dock: a tight row right below it
    _ensureBrowserWatch(root, web, x, y - int(web.nodeHeight) - 30)
    return web, None
...
def EnsureDevRails(root=None):
    ...
    if web is None:
        ...
        _ensureBrowserWatch(root, web, int(web.nodeX),
                            int(web.nodeY) - int(web.nodeHeight) - 30)
    out['browser_watch'] = root.op(BROWSER_WATCH).path
...
BOOTSTRAP_KEEP = (COMP_NAME, 'webBrowser', BROWSER_WATCH)
