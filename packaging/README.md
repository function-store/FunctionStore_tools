...
**Curated in `catalog.json`** — `category` and `description`, the two
things the project genuinely cannot tell us, plus a per-package
`recommended` flag (the CMS checkbox) that puts a tool in the picker's
*Recommended* preset; the generator derives the manifest's `starter` list
from it, tools only. Descriptions and the recommended set were seeded by
...
**The first drop opens the picker by itself.** The root carries
`exec_root_welcome`, an Execute DAT whose `onCreate` fires for every way a
DAT can come into being -- dropped from the `.tox`, loaded with the
project, copied, pasted. Only the drop is a first run, and a storage flag
on the root (`FNS_welcomed`) tells them apart: the shipped root carries no
storage at all (`_bootstrapRoot` unstores everything), the flag is set the
moment the DAT first fires, and ninety frames later -- the installer's
extension and the webBrowser need to come up; the paste rail waits the
same -- it pulses the root's **Pick Tools**, so the routing (console once
core exists, the installer's own picker before) lives in one place. The
dev master never welcomes: its root `externaltox` is set, and so is the
staged build copy's when the DAT is created there. The paste rail stores
the flag right after `loadTox`, so a scripted drop installs its selection
without a picker appearing over it. Like the forwarder, the DAT is
generated by `EnsureRootEntryPoints` and lives in the dev root too.

**The served picker has a first run of its own.** `/manifest.js` reports
`FNS_FIRSTRUN` when the target holds nothing but the rails. The page then
speaks to someone who has never seen the toolkit: a welcome dialog offers
**Set up like last time** when this machine remembers an install (below),
**Recommended** (every tool flagged `recommended` in `catalog.json` -- the
CMS checkbox -- which the manifest carries as `starter`; the page hides
the preset when it is empty or the manifest predates it), **Everything**,
or **Pick my own**; a step
strip tracks choose / adjust / review; and a successful install ends on a
done step whose **Open Settings** posts `/settings`, which pulses the
root's Open Settings so the console takes the panel over. The installer
keeps serving for a minute after an install (not seconds) so that button
has something to answer it.

**"Set up like last time" is the root's own roaming state.** The root is a
config-registry host like any tool (`FNS_ConfigHost`, canonical `FNS`,
shipped in the bootstrap and severed like every stamped host), and its
`config_callbacks` DAT -- generated by `EnsureRootEntryPoints` next to the
forwarder -- returns one `state` entry on every SaveAll: `last_install`,
derived from the rails' `installed` table (packages, project, when, and
the installer's `Package Files` mode). A root with no table keeps the
record another project wrote rather than erasing it. On a bare root the
installer reads the registry's default file directly
(`InstallerExt.LastInstall`; project scope reads nothing) and serves it
as `FNS_LAST`; the page pre-checks the tools the catalog still knows and
offers the card first. Offered, never applied: it lands on the same
Review / Install path. Reasoning and limits: `docs/LastInstallRecord.md`.
...
- **Visibility policy inside.** *Render Only While Window Open* follows
  ...
  inside); both off = `Active` is yours. Each watcher reads its own
  owner: **Window Owner** (a panel COMP -- `winopen` exists only there;
  typically the panel that contains this browser) and **Viewer Owner**
  (any operator -- every op has the flag; e.g. a TOP showing the
  browser's output). Both default to the component itself. A Web Render
  cooks a whole browser process otherwise, and this component ships to
  every user.
...
1. Drop `dist/FNSTools.tox` (from the bucket) into the
   project.
2. The picker opens by itself in the webBrowser panel (first drop only;
   later, the root's **Pick Tools**). First run: the page says it is
   fetching the catalog while the store refreshes, then welcomes with
   **Recommended** / **Everything** / **Pick my own**.
3. Adjust the list, hit **Review install…**, read the plan, hit
   **Install**. Packages land in the dropped container; the done step's
...
`project` is refused in the toolkit's own development project
(`FolderLock`: Private Investigator's **DEV** toggle declares it, and the
default `<project>/FNStools` there IS the externalized source tree); the
plan reports `LOCKED`, on both rails. An explicit **Package Folder**
outside the project folder is still allowed.
