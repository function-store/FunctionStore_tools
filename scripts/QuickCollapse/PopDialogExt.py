...
class PopDialogExt:
	...
	def __init__(self, ownerComp):
		...
		TDF.createProperty(self, 'EnteredText1', value='')
		TDF.createProperty(self, 'EnteredText2', value='')
	...
	def OpenDefault(self, text='', title='', buttons=('OK',), callback=None,
					details=None, textEntry1=False, textEntry2=False, escButton=1,
					escOnClickAway=True, enterButton=1):
		self.Open(text, title, buttons, callback, details, textEntry1, textEntry2, escButton,
				  escOnClickAway, enterButton)

	def Open(self, text=None, title=None, buttons=None, callback=None,
			 			details=None, textEntry1=None, textEntry2=None, escButton=None,
		...
		self.ownerComp.op('entry1/inputText').par.text = self.EnteredText1
		self.ownerComp.op('entry2/inputText').par.text = self.EnteredText2
		self.ownerComp.op('entry1').cook(force=True)
		self.ownerComp.op('entry2').cook(force=True)
	...
	def OnButtonClicked(self, buttonNum):
		...
		if self.ownerComp.par.Textentryarea.eval():
			infoDict['enteredText'] = [self.EnteredText1, self.EnteredText2]
		try:
	...
	def OnKeyPressed(self, key):
		...
		if key == 'tab':
			# focus next entry
			self.ownerComp.op('entry2/inputText').setKeyboardFocus(selectAll=True)
